const positions = [
  {
    company: 'Investree',
    position: 'Fullstack Engineer',
    link: 'https://investree.id/',
    daterange: 'Jan 2022 - Present',
    points: [
      'Designed and implemented new features using VueJS to visualize data stored from APIs, decreasing the time needed by users to find important information about their investments.',
      'Designed and deployed RESTful APIs that served data to the front-end based on dynamically chosen user inputs that handled over 100,000 users using Kotlin.',
      'Created extensive unit tests using Jest to ensure >80% code coverage for existing components and my newly released features, reducing bug reports from users by 32%.'
    ],
  },
  {
    company: 'Singular Asset Management',
    position: 'Equity Analyst',
    link: 'https://singular.com.my/',
    daterange: 'Sep 2021 - Dec 2022',
    points: [
      'Recommended long-term trading opportunities on the Indonesian stock exchange to my portfolio manager and CIO, with the goal of delivering attractive risk-adjusted returns to investors.',
      'Performed company analysis, industry research and competitor analysis to identify trends in the industry.',
      'Conducted DCF analysis on target companies to identify undervalued opportunities.'
    ],
  },
  {
    company: 'Kredens Capital Management',
    position: 'Trading Assistant',
    link: 'https://kredenscapital.com/',
    daterange: 'Feb 2021 - July 2021',
    points: [
      'Recommended short-term trading opportunities on the NASDAQ, NYSE, and LSE stock exchanges to my portfolio manager and CIO using fundamental analysis and technical analysis, generating a 40% increase in profits from my ideas.',
      'Executed trades through multiple channels including electronic execution platforms (e.g., Bloomberg)',
      'Monitored positions daily for opportunities to adjust exposure based on changing market conditions and updated investor reports regularly regarding performance statistics and portfolio holdings.',
      'Provided analytical support to other members of the Investment Team by developing new trading strategies, analyzing existing portfolios, conducting research related to specific industries or companies, etc.'
      
    ],
  },
  {
    company: 'National University of Singapore, Office of Environmental Sustainability',
    position: 'Student Associate',
    link: 'https://sustainability.nus.edu.sg/',
    daterange: 'September 2020 - Jan 2021',
    points: [
      'Tasked with collecting data on recycling rates of a newly developed recycling bin design developed by the College of Engineering',
      'Performed statistical analysis on the data using R and consolidated my findings into a pitch deck for my Associate Director.',
      'Helped secure a SG$5000 for the project from the NUS Directors and close a SG$25,000 purchase deal between CapitaLand and Landlease.'
    ],
  }
];

export default positions;
